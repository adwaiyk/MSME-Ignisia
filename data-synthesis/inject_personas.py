...
Purpose: Inject three deterministic demo personas into the existing synthetic CSVs
         generated by generate_msme_noise.py.
"""
...
def inject_hero_master(master_df: pd.DataFrame) -> pd.DataFrame:
    hero_row = {
        ...
        "gstin": HERO_ID,
        "industry_code": 5065, # manufacturing
        "vintage_months": 14,
...
def inject_hero_gst(gst_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for rp_dt in periods:
        ...
        for return_type in ["GSTR1", "GSTR3B"]:
            ...
            # Hero files exactly on time or early
...
def inject_ghost_upi(upi_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for month_offset in range(6):
        ...
        if month_offset < 5:
            ...
            num_tx = np.random.randint(8, 12)  # 60% drop
    ...
    for month_offset in range(6):
        ...
        for _ in range(num_bills):
            ...
            rows.append({
                ...
                "entity_id": GHOST_ID,
                "linked_upi_txn_id": None,
                "is_anomaly": month_offset >= 5 # Flag the drop off bills as anomaly usually
            })
...
def inject_launderer_upi(upi_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for month_offset in range(6):
        ...
        for _ in range(5):
            ...
            # Node A minimal normal activity
            ring_rows.append({
                "entity_id": LAUNDERER_ID,
                "txn_id": f"TXN{tx_counter:012d}",
                "timestamp": ts.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        for month_offset in range(6):
            ...
            for _ in range(num_bills):
                ...
                rows.append({
                    ...
                    "entity_id": node_id,
                    "linked_upi_txn_id": None,
                    "is_anomaly": True # Flag Launderer generated bills as anomaly highly
                })
